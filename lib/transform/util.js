...
exports.traverse = function(node, fn, parent) {
  ...
  if (node && typeof node === 'object' && node.type) {
    ...
    for (var i = 0; i < node.length; i++) {
      trvResult = exports.traverse(node[i], fn, parent);
      if (trvResult === 'break') {
        return 'break';
      } else if (trvResult === 'delete') {
...
exports.traverseImmediateScope = function(node, fn) {
  if (node.type === "BlockStatement") {
    ...
    return exports.traverse(node, function(node, parent) {
...
exports.letterCounter = function(i) {
  // TODO: Support multiple characters
  if (i >= 26) {
    throw Error("Number is too large");
  }
  return String.fromCharCode(97 + Math.min(25, i));
}