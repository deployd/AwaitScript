...
exports.transform = function(ast) {
  ...
  function parseAsyncScope(node, scope) {
    ...
    node.body.forEach(function(s) {
      if (s.type === "AsyncModifier") {
        ...
        if (scope.type == "FunctionDeclaration") {
          ...
          // Hack to ignore the rest of a function after a return
          currentNode = {body: []}; //dummy
        } else {  
          error(s, "Unexpected return outside of function");
        }
      } else {
        traverse(s, function(n) {
          if (isAwait(n)) {
    ...
    if (scope && scope.type === "FunctionDeclaration") currentNode.body.push({
      type: "ReturnStatement",
      argument: literalStatement("fn && fn();").expression
    });
